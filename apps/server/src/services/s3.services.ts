...
class S3Service {
  ...
  async getPreSignedUploadUrl(
    ...
    try {
      ...
      logger.error(
        `❌ Failed to generate pre-signed URL for upload: ${uploadId}`,
        error
      )
      throw error
    }
  ...
  async deleteFolder(prefix: string) {
    ...
    try {
      ...
      logger.info(
        `✅ Successfully deleted ${keys.length} objects from ${prefix}`
      )
      ...
      logger.error(
        `❌ Critical failure during S3 folder deletion: ${prefix}`,
        error
      )
      throw error
    }
