        ...
        if (response.Messages && response.Messages.length > 0) {
          for (const message of response.Messages) {
            if (message.Body) {
              ...
              logger.info(
                `📥 Received transcoding request from SQS: ${payload.videoId}`
              )
              ...
              try {
                ...
                logger.error(
                  "❌ Database synchronization failed for status: PROCESSING",
                  err
                )
              }
              ...
              try {
                if (isDev) {
                  ...
                  logger.info(
                    `🏠 [DEV] Triggering local Docker worker for: ${payload.videoId}...`
                  )
                  await dockerService.runTask(taskParams)
                  // Update the DB with the local container name as the 'external_id'.
                  await postgresService.query(
                    "UPDATE videos SET external_id = $1 WHERE id = $2",
                    [`transcoder-${payload.videoId}`, payload.videoId]
                  )
                } else {
                  // CLOUD FLOW: Launch an isolated AWS ECS Fargate task.
                  logger.info(
                    `Cloud [PROD] Launching AWS ECS Fargate task for: ${payload.videoId}...`
                  )
