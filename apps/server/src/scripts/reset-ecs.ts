...
if (!region || !accessKeyId || !secretAccessKey) {
  logger.error(
    "❌ Missing AWS credentials. Cannot proceed with infrastructure teardown."
  )
  process.exit(1)
}
  ...
  try {
    ...
    try {
      ...
      await ecsClient.send(
        new DeleteClusterCommand({ cluster: "video-transcoding-cluster" })
      )
    ...
    try {
      ...
      const taskDefsRes = await ecsClient.send(
        new ListTaskDefinitionsCommand({
          familyPrefix: "video-transcoding-task",
        })
      )
