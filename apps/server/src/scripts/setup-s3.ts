...
if (!region || !bucketName) {
  logger.error(
    "❌ Missing AWS credentials or S3 bucket name. Cannot proceed with setup."
  )
  process.exit(1)
}
...
async function setupS3() {
  logger.info(
    `🚀 Starting AWS S3 environment setup for bucket: ${bucketName}...`
  )
  ...
  try {
    ...
    try {
      ...
      exists = true
      logger.info(
        `ℹ️ Bucket '${bucketName}' already active. Updating configuration...`
      )
    } catch (err: any) {
