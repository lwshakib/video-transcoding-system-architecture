...
if (!region || !accessKeyId || !secretAccessKey) {
  logger.error(
    "❌ Missing AWS credentials. Cannot proceed with infrastructure setup."
  )
  process.exit(1)
}
...
async function getOrCreateRole(
  ...
  try {
    ...
    if (!roleRes.Role || !roleRes.Role.Arn) {
      throw new Error(
        `❌ Found role '${roleName}' but its Amazon Resource Name is missing.`
      )
    }
...
async function autoPushDockerImage(repositoryUri: string) {
  ...
  execSync(`docker build -t transcoding-container:latest ${buildContext}`, {
    stdio: "inherit",
  })

  // Step 4: Tag the local image with the remote ECR URI.
  logger.info(`🏷️ Tagging image for remote registry...`)
  execSync(`docker tag transcoding-container:latest ${repositoryUri}:latest`, {
    stdio: "inherit",
  })
...
async function setupECS() {
  logger.info(
    "🚀 Starting end-to-end AWS ECS Fargate environment provisioning..."
  )
  ...
  try {
    ...
    const executionRoleArn = await getOrCreateRole(
      "VideoTranscodingTaskExecutionRole",
      ecsAssumeRolePolicy
    )
    ...
    try {
      ...
      const ecrRes = await ecrClient.send(
        new DescribeRepositoriesCommand({
          repositoryNames: ["transcoding-container"],
        })
      )
      ...
      if (error.name === "RepositoryNotFoundException") {
        ...
        logger.info(
          `🔧 Provisioning new ECR Repository: transcoding-container...`
        )
        const createEcr = await ecrClient.send(
          new CreateRepositoryCommand({
            repositoryName: "transcoding-container",
          })
        )
    ...
    const clusterRes = await ecsClient.send(
      new CreateClusterCommand({ clusterName: "video-transcoding-cluster" })
    )
    ...
    if (!defaultVpcId)
      throw new Error(
        "No default VPC found. ECS Networking cannot be resolved."
      )
    ...
    logger.info(
      `✅ Compute engine and container registry are fully operational.`
    )
  } catch (error) {
