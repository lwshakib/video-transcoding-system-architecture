  ...
  try {
    ...
    const videos = result.rows.map((v) => {
      ...
      return {
        ...
        m3u8_url: isCompleted
          ? getPublicUrl(`${v.id}/transcoded/master.m3u8`)
          : null, // The HLS entry point.
        subtitles_url: isCompleted
          ? getPublicUrl(`${v.id}/subtitles.vtt`)
          : null, // AI-generated captions.
      }
    })
...
app.get("/videos/upload-url", async (req, res) => {
  ...
  if (!fileName || !contentType) {
    return res
      .status(400)
      .json({ error: "Missing required query params: fileName, contentType" })
  }
...
app.get("/videos/:id", async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      previews_url: isCompleted
        ? getPublicUrl(`${v.id}/previews/preview`)
        : null, // Base path for frame previews.
    })
  } catch (error) {
...
app.post("/videos", async (req, res) => {
  ...
  if (!title || !fileName || !contentType) {
    return res
      .status(400)
      .json({ error: "Missing required fields: title, fileName, contentType" })
  }
  ...
  try {
    ...
    if (!finalKey) {
      ...
      await postgresService.query("UPDATE videos SET url = $1 WHERE id = $2", [
        finalKey,
        videoId,
      ])
    } else {
...
app.post("/videos/:id/start", async (req, res) => {
  ...
  try {
    ...
    await postgresService.query(
      "UPDATE videos SET status = 'QUEUED' WHERE id = $1",
      [id]
    )
