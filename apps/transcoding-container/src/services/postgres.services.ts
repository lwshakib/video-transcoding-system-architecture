...
class PostgresService {
  ...
  async setProcessing() {
    ...
    await this.query("UPDATE videos SET status = 'PROCESSING' WHERE id = $1", [
      VIDEO_ID,
    ])
  ...
  async setCompleted() {
    ...
    await this.query("UPDATE videos SET status = 'COMPLETED' WHERE id = $1", [
      VIDEO_ID,
    ])
  ...
  async setFailed() {
    ...
    await this.query("UPDATE videos SET status = 'FAILED' WHERE id = $1", [
      VIDEO_ID,
    ])
