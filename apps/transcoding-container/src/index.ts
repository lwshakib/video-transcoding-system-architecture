...
const transcodeHLS = async (quality: any) => {
  ...
  logger.info(
    `[HLS ${quality.name}] Processing ${quality.width}x${quality.height}...`
  )
  ...
  const args = [
    ...
    "-vf",
    `scale=${quality.width}:${quality.height}`, // Scale to target resolution.
    "-c:v",
    "libx264", // Use H.264 video codec.
    "-profile:v",
    "baseline", // High compatibility profile.
    "-level",
    ...
    "-hls_list_size",
    "0", // Include all segments in the playlist (no sliding window).
    "-f",
...
async function run() {
  try {
    ...
    const [inputWidth, inputHeight] = resOutput
      .split("x")
      .map(Number)
      .filter((n) => !isNaN(n))
    ...
    logger.info(
      `Source Video Insights: ${inputWidth}x${inputHeight}, Duration: ${duration}s`
    )

    // Only transcode to qualities lower than or equal to the source resolution to avoid upscaling artifacts.
    const applicableQualities = TARGET_QUALITIES.filter(
      (q) => q.height <= inputHeight
    )
    ...
    try {
      ...
      await runCommand("ffmpeg", [
        ...
        inputPath,
        "-ss",
        "00:00:01.000", // Capture precisely at the 1-second mark.
        "-vframes",
        "1",
        localThumbPath,
      ])
    ...
    try {
      ...
      await runCommand("ffmpeg", [
        ...
        inputPath,
        "-vf",
        "fps=1/10,scale=160:-1", // One snapshot every 10 seconds, scaled to 160px width.
    ...
    try {
      ...
      logger.error(
        "⚠️ STT Pipeline Failed, generating fallback subtitle...",
        err
      )
      fs.writeFileSync(
        localSubtitlePath,
