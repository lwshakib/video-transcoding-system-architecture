...
export function VideoUpload({
  ...
  const startUpload = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const { data: uploadInfo } = await axios.get(
  ...
  return (
    <div className="w-full px-4">
      ...
      <div
        className={cn(
          "group relative flex flex-col items-center justify-center gap-4 rounded-xl border border-dashed py-16 transition-all duration-200",
          dragActive
            ? "border-primary bg-primary/5" // Active state (Emerald)
            : "hover:bg-zinc-910/50 border-zinc-800 bg-zinc-900/30", // Idle/Hover state
      ...
      >
        ...
        {error && (
          <div className="absolute right-0 -bottom-8 left-0 flex items-center justify-center gap-2 text-xs text-rose-500 transition-all">
