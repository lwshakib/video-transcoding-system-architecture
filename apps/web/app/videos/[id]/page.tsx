...
export default function VideoPage(props: { params: Promise<{ id: string }> }) {
  ...
  const [menuLevel, setMenuLevel] = useState<"main" | "quality" | "speed">(
    "main"
  )
  ...
  const initHls = useCallback(() => {
    ...
    if (windowHls && windowHls.isSupported()) {
      ...
      hls.on(windowHls.Events.LEVEL_SWITCHED, (...args: unknown[]) => {
   ...
   */
  const toggleScrubbingInternal = useCallback(
    (e: React.MouseEvent | MouseEvent, forceState?: boolean) => {
      if (!timelineContainerRef.current || !videoRef.current) return
      const rect = timelineContainerRef.current.getBoundingClientRect()
      const percent =
        Math.min(Math.max(0, e.clientX - rect.x), rect.width) / rect.width
      ...
      const newIsScrubbing =
        forceState !== undefined ? forceState : (e.buttons & 1) === 1
      ...
      if (newIsScrubbing) {
        ...
        if (!wasPaused.current) {
          videoRef.current
            .play()
            .catch((err) => console.error("Playback error after scrub:", err))
        }
      }
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-start bg-zinc-950 px-6 py-20">
        <div className="animate-shimmer aspect-video w-full max-w-[1000px] rounded-3xl border border-white/5 bg-zinc-900/50" />
      </div>
    )
  }
  ...
  const previewImgNumber = Math.max(
    1,
    Math.floor((previewPosition * duration) / 10)
  )
  const previewImgSrc = `${video.previews_url}${previewImgNumber}.jpg`

  return (
    <div className="flex min-h-screen flex-col items-center justify-start overflow-x-hidden bg-zinc-950 px-6 py-12 font-sans text-white">
      ...
      {!isTheater && !isFullscreen && (
        <header className="mb-8 flex w-full max-w-[1000px] items-center justify-between">
          <Link
            href="/"
            className="group flex items-center gap-2 px-2 text-zinc-500 transition-colors hover:text-white"
          >
      ...
      >
        ...
        <div className="video-controls-container">
          ...
          <div className="controls">
            ...
            <button
              className={cn(
                "captions-btn -mr-4 transition-colors",
                isCaptions && "text-rose-500"
              )}
            ...
            <div className="settings-wrapper">
              ...
              >
                <PopoverTrigger asChild>
                  <button
                    className={cn("settings-btn", isSettingsOpen && "active")}
                  >
                    <Settings size={18} />
                  </button>
                </PopoverTrigger>
                <PopoverContent
                  side="top"
                  align="end"
                  className="w-56 border-white/5 bg-zinc-900/90 p-2 backdrop-blur-2xl"
                >
                  {/* MAIN MENU: Select category */}
                  {menuLevel === "main" && (
                    <div className="flex flex-col gap-0.5">
                      <Item
                        className="cursor-pointer border-none py-2 text-white/70 transition-all hover:text-white"
                      ...
                      >
                        ...
                        <ItemActions>
                          <span className="font-mono text-[10px] opacity-40">
                            {currentQuality === -1
                              ? actualQuality !== -1
                                ? `Auto (${hlsLevels[actualQuality]?.height}p)`
                                : "Auto"
                              : `${hlsLevels[currentQuality]?.height}p`}
                          </span>
                          <ChevronRight size={14} className="ml-1 opacity-30" />
                        </ItemActions>
                      </Item>
                      <Item
                        className="cursor-pointer border-none py-2 text-white/70 transition-all hover:text-white"
                  ...
                  {menuLevel === "quality" && (
                    <div className="flex flex-col">
                      <div
                        className="group mb-2 flex cursor-pointer items-center gap-2 rounded-lg px-2 py-2"
      ...
      {!isTheater && !isFullscreen && (
        <div className="mt-8 flex w-full max-w-[1000px] flex-col items-start px-2">
          ...
          <div className="flex items-center gap-3">
            <Badge
              variant="outline"
              className="border-white/10 bg-white/5 px-3 py-1 text-[10px] font-bold tracking-widest uppercase"
            >
              {video.status}
            </Badge>
